...
import { navegarRecepcion } from '../../router/recepcionRouter.js';
import estilos from './RecepcionDashboard.module.css';

export const renderizarPanelRecepcion = (contenedorApp) => {
    ...
    panel.innerHTML = `
        <nav id="recepcionMenu" class="${estilos.menuLateral}">
            ...
            <button data-modulo="membresias" class="${estilos.menuBoton}">Membresias</button>
        </nav>
    ...
    navegarRecepcion('inicio', contenido).catch(error => {
        console.error('Error al cargar el modulo de recepcion:', error);
    });
